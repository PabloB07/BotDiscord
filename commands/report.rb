module Ichigo
  module Commands
    ...
    command(:report, description: "Ichigo Reports - Create a ticket.") do |event, *args|

    next "\\:exclamation: :: go!report [message]" if args.empty?

    ticket = Ichigobot::Database::Ticket.create(
  ...
  command(:ticket, help_available: false,
    ...
    ticket = Ichigobot::Database::Ticket.find(id: t_number.to_i)
  ...
  command( :ticketr, aliases: [:ticketremove,:ticketdelete, :ticketdel], help_available: false, permission_level: 1, permission_message: false) do |event, user|
    ...
    Ichigobot::Database::Ticket.where(user_id: event.message.mentions.first.id).delete
  end

  command(:tickets, help_available: false,
          permission_level: 1, permission_message: false) do |event|

    ticket = Ichigobot::Database::Ticket.where(status: 1).limit(10).reverse_order(:id)
